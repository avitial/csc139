...
/*  Based on the number of threads (T), compute the indices for dividing the array into T equal divisions. 
	For each division i indices[i][0] should be set to the division number i 
	indices[i][1] should be set to the start index
	indices[i][2] should be set to the end index
	For example if the array size (N) is 1000 and T is 2, the indices will be: 
		indices[0][0]=0 
		indices[0][1]=0
		indices[0][2]=499
		indices[1][0]=1
...
void CalculateIndices(int arraySize, int thrdCnt, int indices[MAX_THREADS][3]) {
	...
	for(i=0; i<thrdCnt; i++){
		for(j=0; j<=2; j++){
			if(j==0){
				indices[i][j] = i;
				//printf("indices[%d][%d] value is %d\t",i,0,indices[i][0]);		

			}
			else if(j==1){
				indices[i][j] = (arraySize/thrdCnt)*i;
				//printf("indices[%d][%d] value is %d\t",i,j,indices[i][j]);		
			}
			else if(j==2){
				indices[i][j] = (arraySize/thrdCnt)*(i+1)-1;
				//printf("indices[%d][%d] value is %d\n",i,j,indices[i][j]);		
			}
			else{
				// Do nothing
			}
